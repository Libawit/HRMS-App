...
const PreviewEmployeeModal = ({ employee, isOpen, onClose, onEdit = () => {}, onDelete = () => {},  onDownload = () => {}, theme }) => {
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.header}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => onEdit(employee)} className={`w-12 h-12 rounded-2xl border flex items-center justify-center transition-all ${isDark ? 'bg-white/5 border-white/10 text-slate-400 hover:text-indigo-400' : 'bg-white border-slate-200 text-slate-500 hover:text-indigo-600'}`}>
              <Edit size={20} />
            </button>
            <button onClick={() => onDelete(employee.id)} className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all">
              <Trash2 size={20} />
            </button>
          </div>
        </div>
        ...
        <div className={styles.footer}>
          ...
          <button onClick={() => onEdit(employee)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-indigo-500/20 flex items-center gap-2">
            <Edit size={16} /> Enter Edit Mode
          </button>
        </div>
      </div>
    </div>

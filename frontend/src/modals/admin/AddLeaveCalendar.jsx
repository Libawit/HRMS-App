...
const AddLeaveCalendar = ({ isOpen, onClose, theme = 'dark', onRefresh }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: '',
    endDate: '',
    status: 'APPROVED', // Hardcoded as per request
    reason: '',
    isHalfDay: false
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare the payload - ENSURE IDs ARE STRINGS
    const payload = {
      userId: String(formData.userId),      // Convert to String for Prisma UUID
      leaveTypeId: String(formData.leaveTypeId), // Convert to String for Prisma UUID
      ...
      <div className={`w-full max-w-xl rounded-4xl border ${styles.card} overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200`} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 border-b border-inherit flex items-center justify-between bg-linear-to-r from-transparent to-purple-500/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-xl tracking-tighter text-[#7c3aed]">Log Admin Entry</h3>
              <p className="text-[10px] uppercase tracking-[0.2em] font-bold opacity-50">Directly Approve Absence</p>
            </div>
          </div>
          ...
          <div className="p-8 space-y-6">
                ...
                <div className={`absolute z-50 w-full mt-2 rounded-2xl border max-h-52 overflow-y-auto p-2 ${styles.dropdown}`}>
                  {filteredEmployees.length > 0 ? (
                    filteredEmployees.map(emp => (
                      ...
                      >
                        <div className="w-8 h-8 rounded-lg bg-slate-500/20 flex items-center justify-center text-[10px] font-bold">
                          {(emp.name || emp.firstName)[0]}
                        </div>
                        <div className="text-sm font-black">{emp.name || `${emp.firstName} ${emp.lastName}`}</div>
                      </button>
                    ))
                  ) : (
                    <div className="p-4 text-center text-[10px] font-bold text-slate-500 uppercase">No employees found</div>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
               <div className="text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-500/10 px-3 py-2 rounded-lg border border-emerald-500/20 flex items-center gap-2">
                 ...
                 Deducting: {calculateDays()} Days (Status: APPROVED)
               </div>
            </div>

            {/* Reason */}
            <div className="space-y-1.5">
              <label className={styles.label}>Reason / Internal Comments*</label>
              <textarea 
                rows="3" 
                required
                placeholder="Log internal note for this entry..."

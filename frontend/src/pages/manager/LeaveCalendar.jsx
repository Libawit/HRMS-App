...
const LeaveCalendar = () => {
  ...
  const fetchData = useCallback(async () => {
    if (!user?.departmentId) return; // Prevent fetch if user context isn't ready
      ...
      const [leavesRes, typesRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/auth/leave-requests/calendar`, {
          params: { 
            year, 
            month, 
            status: 'APPROVED', // Manager only sees approved leaves
            deptId: user.departmentId // Strict department scoping
          } 
        }),
  ...
  return (
    <main className={`flex-1 flex flex-col overflow-hidden transition-colors duration-500 ${styles.bgBody}`}>
      ...
      <header className={`p-6 border-b ${styles.border} flex flex-col md:flex-row justify-between items-center gap-4 ${styles.bgCard}`}>
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="bg-[#7c3aed]/20 text-[#7c3aed] text-[9px] font-black px-2 py-0.5 rounded uppercase flex items-center gap-1 tracking-tighter">
              <ShieldCheck size={10} /> Dept Schedule
            </span>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className={`text-[10px] font-black uppercase tracking-widest ${styles.textMuted} mb-4 flex items-center gap-2`}>
                <Layers size={12} /> Leave Types
              </div>
              <div className="space-y-3">
                {leaveTypes.map(type => (
                  <div key={type.id} className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full" style={{backgroundColor: type.color}}></div>
                    <span className={`text-[11px] font-bold ${styles.textMuted}`}>{type.name}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div key={weekIdx} className="relative min-h-35 grid grid-cols-7">
                ...
                <div className="absolute inset-0 pointer-events-none mt-10 px-1 flex flex-col gap-1 overflow-hidden">
                  {filteredEvents.map(event => {
                    ...
                    const baseColor = event.leaveType?.color || '#7c3aed';
      ...
      <ViewLeaveCalendarModal 
        ...
        onEdit={() => handleEditOpen(selectedLeave)}
        theme={theme} 
        data={selectedLeave} 
      />
    </main>

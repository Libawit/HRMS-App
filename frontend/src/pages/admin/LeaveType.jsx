...
import { 
  Plus, Search, ChevronDown, Edit, Trash2, 
...
// External Modals
import AddLeaveTypeModal from '../../modals/admin/AddLeaveType';
import EditLeaveTypeModal from '../../modals/admin/EditLeaveType';
  ...
  return (
    <main className={`flex-1 p-6 lg:p-10 min-h-screen transition-colors duration-500 ${isDark ? 'bg-[#020617]' : 'bg-[#f8fafc]'}`}>
      ...
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        >
          <Plus size={18} strokeWidth={3} /> Add New Leave
        </button>
      </div>
                  ...
                  <tr key={type.id} className="group transition-all">
                    ...
                    <td className={`py-6 px-6 rounded-r-4xl border-y border-r text-right ${isDark ? 'border-white/5 bg-white/1' : 'border-slate-100 bg-white'}`}>
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleOpenView(type)} className={`p-3 rounded-xl border transition-all ${isDark ? 'border-white/5 text-slate-500 hover:text-white hover:bg-[#7c3aed]' : 'border-slate-100 text-slate-400 hover:bg-[#7c3aed] hover:text-white'}`}><Eye size={18} /></button>
                        <button onClick={() => handleOpenEdit(type)} className={`p-3 rounded-xl border transition-all ${isDark ? 'border-white/5 text-slate-500 hover:text-white hover:bg-[#7c3aed]' : 'border-slate-100 text-slate-400 hover:bg-[#7c3aed] hover:text-white'}`}><Edit size={18} /></button>
                        <button onClick={() => handleDelete(type.id)} className={`p-3 rounded-xl border transition-all ${isDark ? 'border-white/5 text-slate-500 hover:text-white hover:bg-red-500' : 'border-slate-100 text-slate-400 hover:bg-red-500 hover:text-white'}`}><Trash2 size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {/* Modals */}
      <AddLeaveTypeModal isOpen={isAddModalOpen} onClose={() => { setIsAddModalOpen(false); fetchLeaveTypes(); }} theme={theme} />
      <EditLeaveTypeModal isOpen={isEditModalOpen} onClose={() => { setIsEditModalOpen(false); fetchLeaveTypes(); }} theme={theme} data={selectedLeave} />
      <ViewLeaveTypeModal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} theme={theme} data={selectedLeave} />
    </main>
  );
};

...
// Keeping only the View Modal for details
...
const LeaveBalance = () => {
  ...
  const [balances] = useState([
    { id: 1, name: "John Smith", email: "john.smith@example.com", type: "Annual Leave", year: 2026, month: 0, alloc: 30.0, used: 2.0, carry: 5.0, avail: 33.0, color: "#7c3aed" },
    { id: 2, name: "John Smith", email: "john.smith@example.com", type: "Sick Leave", year: 2026, month: 0, alloc: 15.0, used: 1.0, carry: 0.0, avail: 14.0, color: "#3b82f6" },
    { id: 3, name: "Sarah Johnson", email: "sarah.johnson@example.com", type: "Annual Leave", year: 2026, month: 0, alloc: 25.0, used: 0.0, carry: 2.0, avail: 27.0, color: "#7c3aed" },
    { id: 5, name: "Alex Thompson", email: "alex.t@example.com", type: "Annual Leave", year: 2026, month: 0, alloc: 20.0, used: 5.0, carry: 0.0, avail: 15.0, color: "#7c3aed" },
  ]);
  ...
  // --- Privacy Logic: Filter for ONLY Current Employee ---
  ...
  }, [searchTerm, balances, currentDate, currentUser.email]);

  const totalAvailable = myBalances.reduce((acc, curr) => acc + curr.avail, 0);
    ...
    <main className={`flex-1 overflow-y-auto p-6 ${styles.bgBody} transition-colors duration-300`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className={`text-sm ${styles.textMuted}`}>Showing leave entitlements for <b>{currentUser.name}</b> in <b>{currentYear}</b></p>
        </div>
        
        <div className="flex items-center gap-3">
          <div className={`flex items-center ${styles.bgInput} border ${styles.border} rounded-xl p-1`}>
            <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear() - 1, 0))} className="p-2 hover:bg-[#7c3aed]/10 text-[#7c3aed] rounded-lg transition-all">
              <ChevronLeft size={18} />
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className={`${styles.bgCard} border ${styles.border} p-6 rounded-4xl flex items-center gap-5`}>
            <div className="w-14 h-14 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500">
                <Wallet size={28} />
            </div>
            <div>
                <p className={`text-[10px] font-black uppercase tracking-widest ${styles.textMuted}`}>Total Available Days</p>
                <h2 className={`text-3xl font-black ${styles.textMain}`}>{totalAvailable.toFixed(1)}</h2>
            </div>
        </div>
        <div className={`md:col-span-2 flex items-center px-6 py-4 rounded-4xl gap-4 border ${isDark ? 'bg-blue-500/5 border-blue-500/10' : 'bg-blue-50 border-blue-100'}`}>
            ...
            <p className={`text-xs leading-relaxed ${styles.textMuted}`}>
                ...
                If you notice any discrepancies, please contact the HR department.
            </p>
        </div>
      </div>

      <div className={`${styles.bgCard} border ${styles.border} rounded-[2.5rem] overflow-hidden`}>
        {/* Search Bar */}
        <div className={`p-6 border-b ${styles.border}`}>
          <div className={`flex items-center ${styles.bgInput} border ${styles.border} px-4 py-3 rounded-2xl gap-3 w-full max-w-md focus-within:border-[#7c3aed] transition-all`}>
            ...
            <input 
              ...
              className="bg-transparent border-none outline-none text-sm w-full text-inherit placeholder:opacity-50"
              value={searchTerm}
              ...
              {myBalances.map((item) => (
                <React.Fragment key={item.id}>
                  ...
                  {activeDetailsRow === item.id && (
                    <tr className={isDark ? "bg-white/2" : "bg-slate-50"}>
                      <td colSpan="6" className={`p-0 border-b ${styles.border}`}>
                        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-top-4 duration-500">
                          ...
                          <div className="flex flex-col justify-center">
                             ...
                             <p className={`text-xs leading-relaxed ${styles.textMuted}`}>
                                Your {item.type} balance is calculated as per the company policy for the {currentYear} fiscal year. 
                                Unused days might be subject to the carry-forward cap of your specific employment contract.
                             </p>
                          </div>
                        </div>
                      </td>
